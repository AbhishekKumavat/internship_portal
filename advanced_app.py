...
# Initialize SocketIO with safe configuration for Vercel
try:
...
@app.route('/')
def hello():
    return jsonify({'message': 'Advanced Flask App - All Components Working!'})

@app.route('/health')
def health():
    db_status = "connected" if db else "not configured"
    socketio_status = "available" if SOCKETIO_AVAILABLE else "not available"
    return jsonify({
        'status': 'healthy', 
        'service': 'internship-portal',
        'database': db_status,
        'socketio': socketio_status,
        'components': ['flask', 'sqlalchemy', 'jwt', 'cors', 'socketio']
    })
...
@socketio.on('connect') if socketio else lambda: None
def handle_connect():
    if socketio:
        emit('connected', {'message': 'Connected to server'})

@socketio.on('disconnect') if socketio else lambda: None
